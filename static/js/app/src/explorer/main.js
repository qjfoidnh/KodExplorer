...
// 	},
    loadMore = function() {
		...
		d = G.jsonData.folderList.length + G.jsonData.fileList.length;
		if (! (c >= d - 1 || "split" == G.userConfig.listType)) {
			d - c > 1e3 ? ($(".init-loading").show(), clearTimeout(loadMoreDelayTimer), loadMoreDelayTimer = setTimeout(function() {
					loadMoreSet(c + 1, d),
					$(".bodymain").scrollTop(e)
				},
				300)) : loadMoreSet(c + 1, d);
		}
	},
	...
	d = function(a) {
		...
		var c = function(a) {
			...
			if (G.jsonData.info && G.jsonData.info.pathType && "" != d[0]) {
				...
				h = '<span class="address-ico">' + core.iconSmall(i.icon) + "</span>",
				g = i.name
			}
			f = f.replace("{$2}", d.length),
			f = f.replace("{$3}", h + '<span class="title-name">' + htmlEncode(g) + "</span>");
			for (var j = f,
			k = 1,
			l = d.length - 1; k < d.length; k++, l--) {
				e += htmlEncode(d[k]) + "/";
				var f = c.replace(/@1@/g, e);
				f = f.replace("{$2}", l),
				f = f.replace("{$3}", '<span class="title-name">' + htmlEncode(d[k]) + "</span>"),
				j += f
			}
			return '<ul class="yarnball">' + j + "</ul>"
		};
		void 0 == a && $("#yarnball").html(c(b)),
		e()
	},
	e = function() {
		$(".yarnball").stop(!0, !0);
